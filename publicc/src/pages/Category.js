...
const Addform = forwardRef((props, ref) => {
	...
	const save = () => {
		let fd = new FormData();
		category.save(data, data.id).then((res) => {
			alert(res.message)
			handleVisible(false);
			list();
		}).catch(err => {
				...
				<Modal.Body>
					...
					<form class="forms-sample">
						<div class="form-group">
							...
							<input type="text" class="form-control" value={data.name || ''} onChange={(e) => { handleChange(e.target.value, 'name') }} placeholder="Category Name" />
						</div>
						<div class="form-group ">
