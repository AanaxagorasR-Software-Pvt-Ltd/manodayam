...
router.post("/new", async (req, res) => {
  ...
  try {
    ...
    if (!resp) {
      ...
      let insertedId = null;
      let MasterCategory = await db.collection("MasterCategory");
      if (body._id) {
        insertedId = await MasterCategory.updateOne(
          { _id: new ObjectID(body._id) },
          { $set: data }
        ).insertedId;
      } else {
        insertedId = await MasterCategory.insertOne(data).insertedId;
      }
...
router.get("/", async (req, res) => {
  try {
    const db = await getDatabase();
    let dt = await db.collection("MasterCategory").find().toArray();
    res.send(dt);
  } catch (err) {
...
router.delete("/delete/:_id", async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const body = req.body;
    let dt = await db.collection("MasterCategory").deleteOne({ _id: _id });
    res.send({
