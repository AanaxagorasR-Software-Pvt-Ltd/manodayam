...
router.post("/new", async (req, res) => {
  ...
  try {
    let resp = await db.collection("shakthi_question").findOne({ ans1: body.ans1 });
    if (resp) {
    ...
    if (!resp) {
      ...
      let insertedId = null;
      let question = await db.collection("shakthi_question");
      if (body._id) {
        insertedId = await question.updateOne(
          { _id: new ObjectID(body._id) },
          { $set: data }
        ).insertedId;
      } else {
        insertedId = await question.insertOne(data).insertedId;
      }
...
router.get("/", async (req, res) => {
  try {
    const db = await getDatabase();
    let dt = await db.collection("shakthi_question").find().toArray();
    res.send(dt);
  } catch (err) {
...
router.delete("/delete/:_id", async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const body = req.body;
    let dt = await db.collection("shakthi_question").deleteOne({ _id: _id });
    res.send({
...
router.post("/shakthi-asking", validate, async (req, res) => {
  const db = await getDatabase();
