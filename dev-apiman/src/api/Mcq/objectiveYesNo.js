...
router.post("/new", async (req, res) => {
  ...
  try {
    let resp = await db.collection("question_yes_no").findOne({ ans1: body.ans1 });
    if (resp) {
    ...
    if (!resp) {
      ...
      let insertedId = null;
      let question_yes_no = await db.collection("question_yes_no");
      if (body._id) {
        insertedId = await question_yes_no.updateOne(
          { _id: new ObjectID(body._id) },
          { $set: data }
        ).insertedId;
      } else {
        insertedId = await question_yes_no.insertOne(data).insertedId;
      }
...
router.get("/", async (req, res) => {
  try {
    const db = await getDatabase();
    let dt = await db.collection("question_yes_no").find().toArray();
    res.send(dt);
  } catch (err) {
...
router.delete("/delete/:_id", async (req, res) => {
  ...
  try {
    const db = await getDatabase();
    const body = req.body;
    let dt = await db.collection("question_yes_no").deleteOne({ _id: _id });
    res.send({
...
router.post("/yesNo", validate, async (req, res) => {
  const db = await getDatabase();
